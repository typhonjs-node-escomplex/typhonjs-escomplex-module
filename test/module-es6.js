...
const s_RUN_TESTS = true;

suite('module-es6:', function () {
    setup(function () {
        this.escomplex = require(modulePath);

        this.parse = function parse(code) {
            var options = esmRegex.test(code) ? esmOptions : scriptOptions;

            //return esprima.parse(code, options);
            return esprima.parse(code, espreeOptions);
        };

        this.analyse = function analyse(code) {
//            console.log('!! module-esm - testParse: ' + JSON.stringify(this.parse(code)));

            var report = this.escomplex.analyse(this.parse(code), mozWalker);

//            console.log('!! module-esm - report: ' + JSON.stringify(report));
    ...
    if (s_RUN_TESTS) {
        suite('require:', function () {
            ...
            test('analyse function is exported', function () {
                assert.isFunction(this.escomplex.analyse);
            });
        });
